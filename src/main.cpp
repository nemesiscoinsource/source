...
uint256 hashGenesisBlock("0x7dee39a6d4fa2c3cab30b5aa2fac341618fe81b05ebd0f772516f1d68653c9af");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NEMESISCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NEMESISCOIN Signed Message:\n";
...
{
    // NEMESISCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 30000000 * 30  / 100 / 50;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NEMESISCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NEMESISCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NEMESISCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7dee39a6d4fa2c3cab30b5aa2fac341618fe81b05ebd0f772516f1d68653c9af");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1424783125;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084826545;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NEMESISCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NEMESISCOINMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running NEMESISCOIN with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NEMESISCOINMiner terminated\n");
        throw;
    }
}
